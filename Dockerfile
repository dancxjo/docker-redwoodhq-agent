...
RUN sed -i 's/AgentPort=[0-9]*/AgentPort='"$AGENT_PORT"'/' properties.conf
RUN sed -i 's/AppServerPort=[0-9]*/AppServerPort='"$APP_SERVER_PORT"'/' properties.conf
RUN sed -i 's/AppServerIPHost=.*/AppServerIPHost='"$APP_SERVER_HOST"'/' properties.conf
...
RUN bash -c 'echo "cd /usr/local/RedwoodHQAgent/agent && ./start.sh" >> /.bashrc'
